...
class BenchmarkResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_delta_mb": (
                round(self.memory_delta / (1024 * 1024), 2) if self.memory_delta else None
            ),
            "memory_peak_mb": (
                round(self.memory_peak / (1024 * 1024), 2) if self.memory_peak else None
            ),
...
class PerformanceBenchmark:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if memory_deltas:
            summary.update(
                {
                    "average_memory_delta_mb": sum(memory_deltas)
                    / len(memory_deltas)
                    / (1024 * 1024),
...
class ExtractionBenchmark(PerformanceBenchmark):
    ...
    def benchmark_extraction(
        self, extractor, extraction_method: str = "extract", **method_kwargs
    ) -> Any:
    ...
    def compare_extractors(
        self, extractors: List[Any], extraction_method: str = "extract", **method_kwargs
    ) -> Dict[str, Any]:
        """Compare performance of multiple extractors."""
        logger.info("Starting extractor performance comparison", extractor_count=len(extractors))
        ...
        for extractor in extractors:
            ...
            try:
                result = self.benchmark_extraction(extractor, extraction_method, **method_kwargs)
        ...
        successful_results = {
            name: data for name, data in comparison_results.items() if data["success"]
        }

        if successful_results:
            fastest = min(
                successful_results.items(), key=lambda x: x[1]["benchmark"]["duration_seconds"]
            )
            slowest = max(
                successful_results.items(), key=lambda x: x[1]["benchmark"]["duration_seconds"]
            )
            ...
            comparison_summary = {
                ...
                "performance_improvement": (
                    ...
                    if fastest[1]["benchmark"]["duration_seconds"] > 0
                    else 1
                ),
            }
        else:
...
def benchmark_jira_queries(
    ...
    logger.info(
        "Starting Jira query benchmarks",
    ...
    return {"query_results": results, "overall_summary": benchmark.get_summary()}
