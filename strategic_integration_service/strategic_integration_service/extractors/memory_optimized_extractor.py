...
from ..models.initiative import L2Initiative, CurrentInitiative
...
class MemoryOptimizedL2Extractor(L2InitiativeExtractor):
    ...
    def extract_streaming(
        ...
        with memory_monitoring("l2_streaming_extraction") as monitor:
            logger.info("Starting streaming L2 extraction",
            ...
            for issue_data in loader:
                ...
                try:
                    ...
                    logger.error("Failed to parse issue", 
                               issue_key=issue_key, 
                               error=str(e))
    ...
    def extract_chunked(
        ...
        with memory_monitoring("l2_chunked_extraction") as monitor:
            logger.info("Starting chunked L2 extraction",
    ...
    def extract_all_optimized(
        ...
        with memory_monitoring("current_initiatives_extraction") as monitor:
            ...
            for query_name, jql in queries:
                ...
                logger.info(f"Completed {query_name} extraction", 
                          count=len(results[query_name]))
            
            logger.info("Memory-optimized extraction completed",
...
def compare_memory_strategies(
        ...
        with memory_monitoring(f"strategy_{strategy}") as monitor:
            try:
                if strategy == "streaming":
                    ...
                    initiatives = list(extractor.extract_streaming(
                        batch_size=50, 
    ...
    if successful_strategies:
        ...
        memory_efficient = min(
            ...
            key=lambda x: x[1]["memory_report"]["peak_mb"]
        )
        
        # Find fastest (assuming lower peak memory correlates with speed)
        fastest = min(
            successful_strategies.items(),
            key=lambda x: len(x[1]["memory_report"]["checkpoints"])
        )
