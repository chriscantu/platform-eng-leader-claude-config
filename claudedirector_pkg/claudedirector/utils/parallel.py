...
class ParallelProcessor:
    ...
    def _process_files_concurrent(
        ...
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=self.max_workers,
            thread_name_prefix="claudedirector_worker"
        ) as executor:
    ...
    def process_content_parallel(
        ...
        try:
            ...
            return {
                ...
                'items_per_second': len(content_items) / processing_time if processing_time > 0 else 0
            }
            
        except Exception as e:
            logger.error("Parallel content processing failed", error=str(e))
            raise ClaudeDirectorError(f"Parallel processing failed: {e}", component="parallel_processor")
    
    def _process_content_batch(
        self,
        content_batch: List[Tuple[str, Dict[str, Any]]],
        processor_func: Callable
    ) -> List[Any]:
