# Procore Jira Integration Setup
# Director of Engineering - UI Foundation Platform

## Quick Setup for Procore

### 1. Generate Procore Jira API Token

1. **Access Procore Jira**: Navigate to https://procoretech.atlassian.net
...
```bash
# Procore Jira Integration for Weekly Reports
export JIRA_BASE_URL="https://procoretech.atlassian.net"
...
1. Go to https://procoretech.atlassian.net/projects
...
2. **API Discovery**:
   ...
   curl -u chris.cantu@procore.com:$JIRA_API_TOKEN \
     -X GET \
     "https://procoretech.atlassian.net/rest/api/3/field" | jq '.[] | select(.name | contains("value") or contains("impact") or contains("roi"))'
   ```
